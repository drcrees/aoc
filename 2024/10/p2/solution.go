...
func Solve() {
	...
	grid := helpers.ReadInts("./2024/10/p2/input")
...
func hike(coords Coords, grid [][]int) int {
	result := 0
	...
	if inbounds(coords.x, coords.y, -1, 0, grid) && grid[coords.y][coords.x-1]-current == 1 {
		result += hike(Coords{coords.x - 1, coords.y}, grid)
	}
	...
	if inbounds(coords.x, coords.y, 0, -1, grid) && grid[coords.y-1][coords.x]-current == 1 {
		result += hike(Coords{coords.x, coords.y - 1}, grid)
	}
